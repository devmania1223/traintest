...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  Stream<LoginState> _mapLoginWithCredentialsPressedToState({
    ...
    try {
      ...
      storeUserData(auth.token, auth.id, auth.email, auth.username);
  ...
  void storeUserData(String token, int id, String email, String name) async {
    Prefer.prefs = await SharedPreferences.getInstance();
    Prefer.prefs.setString('token', token);
    Prefer.prefs.setInt('id', id);
    Prefer.prefs.setString('email', email);
